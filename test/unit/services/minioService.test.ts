...
describe("MinIoService", () => {
    ...
    it("getPresignedUrl should return url", async () => {
        mockMinioClientForPresigned.presignedGetObject.mockResolvedValue("http://url");
    ...
    it("getPresignedUrl should return empty string on error", async () => {
        mockMinioClientForPresigned.presignedGetObject.mockRejectedValue(new Error("Fail"));
        const url = await MinIoService.getPresignedUrl("key");
        expect(url).toBe("");
    });
    ...
    it("copyFile should get, stat, and put", async () => {
        ...
        mockMinioClient.statObject.mockResolvedValue({ metaData: { "content-type": "text/plain" } });
    ...
    it("uploadUserProfilePhoto uploads to profile bucket", async () => {
        ...
        const file = { originalname: "p.jpg", mimetype: "image/jpeg", buffer: Buffer.from("data") } as any;
    ...
    it("uploadUserProfilePhoto throws if no file", async () => {
        await expect(MinIoService.uploadUserProfilePhoto(1, undefined as any)).rejects.toThrow();
    });
});