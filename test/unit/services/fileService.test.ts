...
describe("FileService", () => {
  ...
  describe("validateTempFiles", () => {
    ...
    it("should throw if file not found in DB", async () => {
      ...
      await expect(FileService.validateTempFiles(["missing"])).rejects.toThrow(
        "not found"
      );
    });
  ...
  describe("moveToPermanent", () => {
    ...
    it("should throw if file not found", async () => {
      mockTempRepo.findByFileId.mockResolvedValue(null);
      await expect(FileService.moveToPermanent("uid", "perm")).rejects.toThrow(
        "not found"
      );
    });
  });
  ...
  describe("moveMultipleToPermanent", () => {
    it("should move all files", async () => {
      const files = [
        { fileId: "1", permanentPath: "perm/1" },
        { fileId: "2", permanentPath: "perm/2" },
      ];
      mockTempRepo.findByFileId.mockImplementation(
        async (id) => ({ id: Number(id), tempPath: `temp/${id}` }) as any
      );
    ...
    it("should rollback if one fails", async () => {
      ...
      mockTempRepo.findByFileId.mockResolvedValueOnce({
        id: 1,
        tempPath: "temp/1",
      } as any);
      mockTempRepo.findByFileId.mockResolvedValueOnce(null);

      await expect(FileService.moveMultipleToPermanent(files)).rejects.toThrow(
        "not found"
      );
  ...
  describe("deleteTempFile", () => {
    it("should delete from MinIO and DB", async () => {
      mockTempRepo.findByFileId.mockResolvedValue({
        id: 1,
        tempPath: "p",
      } as any);
