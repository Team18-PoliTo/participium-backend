...
describe("EmailVerificationController", () => {
  ...
  describe("verifyEmail", () => {
    ...
    it("handles 'Citizen not found' as 404", async () => {
      req = { body: { email: "test@test.com", code: "123456" } };
      mockCitizenService.verifyEmail.mockRejectedValue(
  ...
  describe("resendVerificationCode", () => {
    it("returns 200 on success", async () => {
      ...
      mockCitizenService.resendVerificationCode.mockResolvedValue({
        message: "Sent",
      });
      ...
      expect(mockCitizenService.resendVerificationCode).toHaveBeenCalledWith(
        "test@test.com"
      );
      expect(res.status).toHaveBeenCalledWith(200);
    });
    ...
    it("handles 'Citizen not found' as 404", async () => {
      req = { body: { email: "test@test.com" } };
      mockCitizenService.resendVerificationCode.mockRejectedValue(
        new Error("Citizen not found")
      );
    ...
    it("handles known errors as 400", async () => {
      ...
      for (const msg of errors) {
        mockCitizenService.resendVerificationCode.mockRejectedValueOnce(
          new Error(msg)
        );
        await controller.resendVerificationCode(
          req as Request,
