...
describe("AuthController", () => {
  ...
  const CitizenRepositoryMock = jest.requireMock(
    "../../../src/repositories/implementation/CitizenRepository"
  ...
  it("loginCitizen returns access token", async () => {
    ...
    citizenService.login.mockResolvedValue({
      access_token: "abc",
      token_type: "bearer",
    });
    const req = {
  ...
  it("loginInternal returns access token", async () => {
    const controller = buildController();
    internalService.login.mockResolvedValue({
      access_token: "xyz",
      token_type: "bearer",
    });
    const req = {
  ...
  it("me returns 404 when citizen record missing", async () => {
    const controller = buildController();
    const req = {
      auth: { kind: "citizen", email: "missing@city.com" },
    } as unknown as Request;
    const res = mockRes();
  ...
  it("me uses default repositories when not provided", async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      jest.isolateModules(() => {
        try {
          const {
            default: IsolatedAuthController,
          } = require("../../../src/controllers/authController");
          const controller = new IsolatedAuthController(
            citizenService,
            internalService
          );
          const req = {
            auth: { kind: "citizen", email: "default@city.com" },
          } as unknown as Request;
          const res = mockRes();
          ...
          controller
            ...
            .then(() => {
              try {
                ...
                const citizenRepoInstance =
                  CitizenRepositoryMock.mock.results[
                    CitizenRepositoryMock.mock.results.length - 1
                  ].value;
                expect(citizenRepoInstance.findByEmail).toHaveBeenCalledWith(
                  "default@city.com"
                );
                expect(res.status).toHaveBeenCalledWith(200);
                resolve();
  ...
  it("me forwards repository errors to next", async () => {
    const controller = buildController();
    const req = {
      auth: { kind: "citizen", email: "err@city.com" },
    } as unknown as Request;
    const res = mockRes();
