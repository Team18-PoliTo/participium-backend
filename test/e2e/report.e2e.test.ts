  ...
  default: {
    ...
    validateTempFiles: jest.fn().mockResolvedValue([
      ...
      },
      {
        ...
        originalName: "photo3.png",
        tempPath: "temp/3.png",
      },
    ]),

    moveMultipleToPermanent: jest
      .fn()
      .mockResolvedValue([
...
describe("Report E2E Tests", () => {
    ...
    const categories = [
      {
        ...
        description: "Strade e Arredi Urbani",
      },
  ...
  it("should create a report with 3 photos", async () => {
    const categoryRepo = AppDataSource.getRepository(CategoryDAO);
    const roadsCategory = await categoryRepo.findOne({
      where: { name: "Roads and Urban Furnishings" },
    });
  ...
  it("should retrieve assigned reports in map area", async () => {
    ...
    await AppDataSource.getRepository(ReportDAO).update(reportId, {
  ...
  it("should reject non-image upload", async () => {
    ...
    fileServiceMock.uploadTemp.mockImplementationOnce(() => {
      throw new Error(
        "File type undefined is not allowed. Allowed types: image/jpeg, image/jpg, image/png, image/gif, image/webp"
      );
    });
  ...
  it("should delete photos from MinIO (logic check)", async () => {
    const categoryRepo = AppDataSource.getRepository(CategoryDAO);
    const lighting = await categoryRepo.findOne({
