...
class FileService {
  ...
  constructor(
    tempFileRepository: TempFileRepository = new TempFileRepository()
  ) {
    ...
    for (const fileId of fileIds) {
      ...
      if (tempFile.expiresAt < now) {
        throw new BadRequestException(
          `Temp file with ID ${fileId} has expired`
        );
      }
      ...
      if (!exists) {
        throw new NotFoundException(
          `Temp file with ID ${fileId} not found in storage`
        );
      }
  ...
  async moveMultipleToPermanent(
    moves: Array<{ fileId: string; permanentPath: string }>
  ): Promise<string[]> {
    const movedPaths: string[] = [];

    try {
      for (const move of moves) {
        const permanentPath = await this.moveToPermanent(
          move.fileId,
          move.permanentPath
        );
  ...
  async cleanupExpiredTempFiles(): Promise<number> {
    ...
    for (const file of expiredFiles) {
      try {
        ...
        if (!deleted) {
          console.error(
            `Failed to delete expired file from MinIO: ${file.tempPath}`
          );
        }
